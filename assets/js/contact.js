$(document).ready(() => {
  ...
  form.submit(event => {
    ...
    subtle
        .importKey(
            'jwk',  // can be 'jwk' (public or private), 'spki' (public only),
                    // or 'pkcs8' (private only)
            {
              // this is an example jwk key, other key types are Uint8Array
              // objects
              kty: 'RSA',
              e: 'AQAB',
            ...
            false,  // whether the key is extractable (i.e. can be used in
                    // exportKey)
        ...
        .then(publicKey => {
          ...
          form.children('input, textarea').each((i, element) => {
            ...
            promises.push(subtle.encrypt(
                {name: 'RSA-OAEP'}, publicKey, enc.encode(elem.val())));
          });
